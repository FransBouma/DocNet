...
namespace MarkdownDeep
{
    public class BlockProcessor : StringScanner
    {
        ...
        private bool HandleDocNetExtension(Block b)
        {
            ...
            if (DoesMatch("@startuml"))
            {
                return HandleUmlExtension(b);
            }
            return false;
        }
        ...
        private bool HandleUmlExtension(Block b)
        {
            int startContent = this.Position;

            // skip @snippet
            if (!SkipString("@startuml"))
            {
                return false;
            }
            if (!SkipLinespace())
            ...
            if (!SkipString("@enduml"))
            {
                return false;
            }

            var umlFolderName = "uml";
            var umlText = this.Input.Substring(startContent, this.Position - startContent);
            var imageFile = Path.Combine(m_markdown.DestinationDocumentLocation ?? string.Empty, umlFolderName, filename);

            var umlFolder = Path.GetDirectoryName(imageFile);
            if (!Directory.Exists(umlFolder))
                Directory.CreateDirectory(umlFolder);

            var srcFile = Path.ChangeExtension(imageFile, "txt");

            File.WriteAllText(srcFile, umlText);

            var jarPath = @"lib\plantuml.jar";

            var clientProcess = new Process();
            clientProcess.StartInfo.FileName = "javaw";
            clientProcess.StartInfo.Arguments = @"-splash: -jar " + jarPath + " \"" + srcFile + "\"";
            clientProcess.Start();

            // When we want the resizer to handle these images 
            // we need to wait for them to be generated
            clientProcess.WaitForExit();

            var imgTag = new HtmlTag("img");
            imgTag.attributes["src"] = Path.Combine(umlFolderName, filename);
